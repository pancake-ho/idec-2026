...
// ============================================================================
// Baseline vs PFDW+FC direct-decision comparison for 1000 images
//
...
module top_tb_cycle_far_vs_baseline;
    ...
    initial begin
        ...
        $display("============================================================");
        $display(" Baseline vs PFDW V19 FAR: 1000-image cycle comparison");
        ...
        for (sample_idx = 0; sample_idx < 1000; sample_idx = sample_idx + 1) begin
            ...
            if (sample_idx == 0) begin
                ...
                $display(" Baseline : %0d cycles  (%0t ps)", base_cycles,
                         base_done_time - image_start_time);
                $display(" PFDW FAR : %0d cycles  (%0t ps)", pfdw_cycles,
        ...
        $display(" Baseline cycles    : avg=%0.2f min=%0d max=%0d",
                 base_cycles_sum / 1000.0, base_cycles_min, base_cycles_max);
        $display(" PFDW FAR cycles    : avg=%0.2f min=%0d max=%0d",
        ...
        $display(" Baseline correct : %0d / 1000  (%0.2f %%)",
        ...
        if (match_count == 1000)
            $display(" RESULT: PASS - All 1000 classification decisions are identical.");
        else
    ...
    initial begin
        #90000000; // 90 us
        $display("[CYCLE COMPARE FAR] TIMEOUT at %0t ps", $time);
        $finish;
    end
