...
// =============================================================================
// PFDW V19 FAR exact signed radix-4 Booth multiplier.
// Arithmetic is inherited bit-for-bit from V18A correction-row Booth.
//
// Difference from V17:
...
// =============================================================================
// PFDW V19 FAR range-proven 49-lane radix-4 Booth arithmetic engine.
//
// V17 starts from V14 and preserves its scheduler, pipeline boundaries, exact lane widths and fixed-point behavior.
// V19 FAR keeps V18A arithmetic and pipeline latency.  The scheduler guarantees
// that convolution requests arrive in output-channel triples sharing one
// activation tile.  FC requests share one activation vector across ten classes.
// The activation banks are therefore written/transformed only on oc==0 (or
// class==0), while the weight path still advances every request.  This is exact
// fan-out-aware transform memoization; no value, scale, or cycle is changed.
//
...
// =============================================================================
module pfdw_pipe49_booth_engine_v19_far (
    input  logic clk,
    input  logic rst_n,
    ...
    // pipeline boundary or request cadence. V19 FAR additionally freezes p0_a
    // for the second/third consumer of a convolution group and for FC classes
    // 1..9.  p0_b is still loaded on every request.
    // -------------------------------------------------------------------------
    ...
    generate
        for (gm = 0; gm < 49; gm = gm + 1) begin : G_V14_LANE
            ...
            always_ff @(posedge clk) begin
                if (p1_valid) begin
                    // The activation operand is identical for the three
                    // output-channel consumers.  FC activations are identical
                    // for all ten class requests.  Freeze the bank otherwise.
                    if ((p1_is_fc && (p1_tag[3:0] == 4'd0)) ||
                        (!p1_is_fc && (p1_tag[3:2] == 2'd0)))
